/*
	share v1.1.11更新公告： 
	1. 新增历史交易记录功能（记录买卖时间/数量/价格/盈亏）
	2. 持仓显示增加「盈亏百分比」字段
	3. 存档/读档支持历史交易数据
	4. 更多功能菜单新增「查看交易记录」选项（编号6）
*/ 
...
// 关键修改1：扩展龙虎榜数据结构，增加股票索引字段
struct StockChange {
    string name;
    double change;
    int index;  // 新增：记录股票在share数组中的索引（0-19）
};
...
void show_holdings() {
    ...
    for (int i = 0; i < 20; i++) {
        if (s_have[i].hand > 0) {
            ...
            double profit_pct = ((current_price - avg_cost) / avg_cost) * 100;  // 盈亏百分比
            double profit = s_have[i].hand * (current_price - avg_cost);       // 盈亏金额
            
            printf("%2d: %-8s 持有 %4d 股 | 成本价: %6.2f | 当前价: %6.2f | 盈亏: %+6.2f%% (%+8.2f元)\n",
                   i, share[i].name.c_str(), s_have[i].hand, avg_cost, current_price, 
                   profit_pct, profit);  // 新增盈亏百分比和金额显示
        }
    }
    cout<<endl; 
...
// 关键修改2：行情显示增加明确操作提示
void show_market() {
    cout << "【操作提示】看到感兴趣的股票后，直接输入表格中的序号（0-19）即可进行买卖\n";
    for (int i = 0; i < 20; i++) {
...
void trade_stock() {
    ...
    if (action == 1) {
        ...
        if (total > pl_money) {
            ...
            record.price = price * 1.0003;  // 包含手续费的成本价
            record.trade_day = current_day;
            record.profit_pct = 0;  // 买入时无盈亏
            record.profit = 0;
            tradeHistory.push_back(record);  // 添加到交易记录
        }
    } else {
        ...
        if (shares > s_have[stock_idx].hand) {
            ...
            // 计算盈亏数据
            double sell_price = price * 0.995;  // 扣除手续费的实际收入
            ...
            record.stock_idx = stock_idx;
            record.shares = shares;
            record.price = price;  // 实际成交价格（未扣手续费前）
            record.trade_day = current_day;
...
void dt() {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        if (change > 0) {
            ...
            gainers[gainer_count].index = i;  // 记录股票索引
            gainer_count++;
        } else if (change < 0) {
            losers[loser_count].name = share[i].name;
            losers[loser_count].change = -change;
            losers[loser_count].index = i;  // 记录股票索引
            loser_count++;
        }
    }
...
void save() {
    ...
    // 写入每条交易记录
    for (const auto& rec : tradeHistory) {
