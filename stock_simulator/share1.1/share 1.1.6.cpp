...
void save() {
    ofstream file("game.sav", ios::binary);
    if (!file) {
        error1("无法创建存档文件！");
        return;
    }
    ...
    // 保存自选股
    file.write(reinterpret_cast<const char*>(&my_stocks_count), sizeof(my_stocks_count));
    for (int i = 0; i < my_stocks_count; ++i) {
        file.write(reinterpret_cast<const char*>(&my_stocks[i]), sizeof(int));
    }
...
void read() {
    ifstream file("game.sav", ios::binary);
    if (!file) {
        error1("未找到存档文件！");
        return;
    }
...
int main() {
    ...
    while (true) {
        ...
        while(true){
	        ...
	        show_holdings();
	        show_market();
	        cout << "\n[1] 进行交易 [2] 跳过今日 [3]更多功能[4]存档[5]读档: ";
	        int choice;
	        flag=0;
	        cin >> choice;
